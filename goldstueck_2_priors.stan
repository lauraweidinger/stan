// alpha/kappa/beta loop inside learning loop
      ...
      preds[it+1,is,2]= preds[it,is,2];
        ...
        for (it in 1:ntr){
          if (resp[it,is]!=999){
           ...
           // probabilities aus der softmax!

        //   resp[it,is] ~ bernoulli_logit(exp(beta[is,1]*preds[it,is,(resp[it,is]+1)])/(exp(beta[is,1]*preds[it,is,(resp[it,is]+1)])+ exp(beta[is,1]*preds[it,is,(2-resp[it,is])])));
          //resp[it,is] ~ log(exp(beta[is,1])*preds[it,is,(resp[it,is]+1)])/(exp(beta[is,1])*preds[it,is,(resp[it,is]+1)])+ exp(beta[is,1]*(2-resp[it,is])))); // softmax
              }
          }
        }
}
