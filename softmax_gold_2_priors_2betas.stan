...
// beta depends on outcome from previous trial.
// outc = 0 = punishment. beta(1) = following punishment.
// outcome = 1 = reward. beta(2) = following reward.
          if (outc[it-1,is]==0){ // punished trial
            real prob = exp(beta[is,1]*preds[it,is,2])/(exp(beta[is,1]*preds[it,is,2])+ exp(beta[is,1]*preds[it,is,1]));
          }
          else if (outc[it-1,is]==1){ // rewarded trial
            real prob = exp(beta[is,2]*preds[it,is,2])/(exp(beta[is,2]*preds[it,is,2])+ exp(beta[is,2]*preds[it,is,1]));
          }
      //    real part = beta[is,(outc[it-1,is]+1)]*preds[it,is,2];
      //    real prob = exp(part)/(exp(part)+ exp(beta[is,(outc[it-1,is]+1)]*preds[it,is,1]));
      // no need to update the beta that wasn't chosen as beta is not a trial by trial trajectory.
